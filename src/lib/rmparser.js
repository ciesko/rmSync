...
function parseRmFile(buffer) {
  ...
  // Walk parent chain; a node is hidden if it or any ancestor is hidden.
  const ROOT_KEY = '0:1';
  ...
  // ── Filter: only strokes from visible layers ──
  const result = [];
...
/**
 * Parse a scene-item block (types 0x03, 0x04, 0x05, 0x08).
 *
